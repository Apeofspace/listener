...
class listenerThread(Thread):
    ...
    def __init__(self, queue, messagequeue, baud=500000):
        self.queue = queue
        ...
        if p is not None:
            try:
                ...
                self.messagequeue.put("Подключено к: {}".format(self.ser.portstr))
                # print("Подключено к: {}".format(self.ser.portstr))
            except serial.SerialException as e:
                self.messagequeue.put('Не удалось подключиться к порту: {}\n'.format(e))
    ...
    def autoFindSerialPort(self):
        ...
        self.messagequeue.put("Поиск COM портов")
        # print("Поиск COM портов")
        for p in ports:
            self.messagequeue.put(" - " + p.description)
            # print(" - " + p.description)
            if "STMicroelectronics Virtual COM Port" in p.description:
                stms.append(p)
        if len(stms) == 1:
            self.messagequeue.put("Автоподключение " + stms[0].name)
            # print("Автоподключение " + stms[0].name)
            return stms[0].name
        if ports:
            while (True):
                result = tkinter.simpledialog.askinteger("Подключение COM", "Укажите какой порт использовать использовать\nНапример: COM5\n")
                if (result[0:3] == 'COM' and result[3].isdigit()):
                    return result
        self.messagequeue.put('Не найдено COM')
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    self.messagequeue.put("Очередь полна, что-то пошло не так! Закрыть поток!")
                    # print("Очередь полна, что-то пошло не так! Закрыть поток!")
                    self.stop()
            except Exception as e:
                self.messagequeue.put('Error reading/decoding line: {}'.format(e))
...
listener.start()
exporter.start()
msgq.put('Запись в файл: {}'.format(exporter.filename))
msgq.put('Потоки запущены. Нажмите любую клавишу для остановки...')
